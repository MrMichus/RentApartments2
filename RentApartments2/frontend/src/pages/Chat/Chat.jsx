...
const Chat = () => {
  ...
  const openChatWithUser = async (userId) => {
    try {
      // Sprawdź czy użytkownik nie próbuje otworzyć czatu z samym sobą
      if (user && user.userId === parseInt(userId)) {
        setError("Nie możesz rozpocząć czatu z samym sobą");
      ...
      const errorMessage =
        err.response?.data?.error || "Nie udało się otworzyć konwersacji";
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.error || "Błąd podczas wysyłania wiadomości";
      setError(errorMessage);
      console.error(err);
    } finally {
  ...
  const getOtherUserName = (chat) => {
    // Backend zawsze ustawia recipientName na drugiego użytkownika
    // (nie na bieżącego użytkownika), więc zawsze zwracamy recipientName
    return chat.recipientName || "Nieznany użytkownik";
  };
              ...
              conversations.map((conv) => (
                ...
                >
                  ...
                  <div className="conv-info">
                    <span className="conv-name">{getOtherUserName(conv)}</span>
                    <span className="conv-preview">
                      {conv.wiadomosci && conv.wiadomosci.length > 0
                        ? conv.wiadomosci[
                            conv.wiadomosci.length - 1
                          ].tresc.substring(0, 30) + "..."
                        : "Brak wiadomości"}
                    </span>
                  </div>
                  {conv.updatedAt && (
          ...
          {currentChatData ? (
            <>
              ...
              <div className="messages-container">
                {!currentChatData.wiadomosci ||
                currentChatData.wiadomosci.length === 0 ? (
                  ...
                  currentChatData.wiadomosci.map((msg, index) => (
                    <div
                      key={index}
                      className={`message ${
                        Number(msg.userId) === Number(user?.userId)
                          ? "sent"
                          : "received"
                      }`}
                    >
            ...
            <div className="no-chat-selected">
              ...
              <p>
                Wybierz rozmowę z listy lub przejdź do mieszkania i napisz do
                właściciela
              </p>
            </div>
          )}
