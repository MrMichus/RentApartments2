...
const AddMieszkanie = ({ editMode = false }) => {
    ...
    const removeZdjecieField = (index) => {
        ...
        setFormData({ ...formData, zdjeciaUrls: newZdjecia.length ? newZdjecia : [''] });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const filteredZdjecia = formData.zdjeciaUrls.filter(url => url.trim() !== '');
            ...
            if (editMode && id) {
                ...
                setSuccess('Ogłoszenie zostało dodane i czeka na zatwierdzenie przez administratora!');
            }
            setTimeout(() => navigate('/moje-mieszkania'), 2000);
        } catch (err) {
            setError(err.response?.data?.message || 'Błąd podczas zapisywania ogłoszenia');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="add-mieszkanie-page">
            <div className="form-container">
                <div className="form-header">
                    ...
                    <p>{editMode ? 'Zaktualizuj dane swojego mieszkania' : 'Wypełnij formularz, aby dodać mieszkanie do wynajęcia'}</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="mieszkanie-form">
                    ...
                    <div className="form-section">
                        <h3>📷 Zdjęcia</h3>
                        <p className="section-description">Dodaj linki do zdjęć mieszkania (opcjonalne)</p>
                    ...
                    {!editMode && (
                        <div className="form-info">
                            <span>ℹ️</span>
                            <p>Po dodaniu ogłoszenie będzie czekać na zatwierdzenie przez administratora.</p>
                        </div>
                    )}
                    ...
                    <div className="form-actions">
                        ...
                        <button type="submit" className="btn-submit" disabled={loading}>
                            {loading ? 'Zapisywanie...' : (editMode ? 'Zapisz zmiany' : 'Dodaj ogłoszenie')}
                        </button>
                    </div>
                </form>
            </div>
